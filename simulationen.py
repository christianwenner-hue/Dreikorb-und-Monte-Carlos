...
def simuliere_historie(df, s_jahr, alt_s, k1_s, k2_s, k3_s, r_std, r_a, alt_a, r_b, alt_b, g_s, infl_pa, k1_j, k2_j, schwelle):
    ...
    # Start-Setup für die echten Gewinne laut README
    kaufwert = k3 * (1 - g_s)
    g_akt = g_s
    ...
    for m, (_, row) in enumerate(df_slice.iterrows()):
        ...
        # Echten absoluten Gewinn berechnen (wie in README gefordert)
        gewinn = k3 - kaufwert
        g_akt = gewinn / k3 if gewinn > 0 and k3 > 0 else 0
        ...
        # SCHMERZGRENZE-CHECK (Gewinn > Jahresrente * Schwelle)
        if gewinn > (n_b * 12 * schwelle) and k3 > 0:
            status = "Normal"
            b_r, _ = berechne_brutto_und_steuer(n_b, g_akt)
            k3 -= b_r
            kaufwert -= b_r * (1 - g_akt) # Kaufwert sinkt anteilig bei Verkauf
            brutto_monat = b_r
...
def run_monte_carlo(n, alt_s, alt_z, k1_s, k2_s, k3_s, r_std, r_a, alt_a, r_b, alt_b, g_s, ret, vol, infl, k1_j, k2_j, schwelle, seed=True, method="math", h_df=None):
    ...
    for i in range(n):
        ...
        if alive: success = success + 1 if 'success' in locals() else 1
    return results, results_brutto, (success/n if 'success' in locals() else 0)
